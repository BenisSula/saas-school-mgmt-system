...
function resolveGrade(score: number, scales: GradeScale[]): { grade: string | null; remark: string | null } {
...
export async function computeStudentResult(
  ...
  const classAverage =
    leaderboard.reduce((sum, row) => sum + row.average, 0) /
